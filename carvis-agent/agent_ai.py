...
def fetch_new_messages(bearer_token, session_id, client):
    ...
    while not flag:
        ...
        try:
            ...
            response = requests.get(f"https://agentverse.ai/v1beta1/engine/chat/sessions/{pathParameters['session_id']}/responses", headers={
                "Authorization": f"bearer {bearer_token}"
            })
...
def look_for_charging_station(bearer_token, email, requestedModel, text_to_speech_client):
...
def act_on_user_answer(find_charging_station, num_tries, bearer_token, email, requestedModel, text_to_speech_client, speech_to_text_client):
    if find_charging_station == "yes" or find_charging_station == "Yes":
        look_for_charging_station(bearer_token, email, requestedModel, text_to_speech_client)
        ...
        display_in_cockpit(decline_message)
        ...
        if num_tries < 2:
            ...
            display_in_cockpit(invalid_message)
            ...
            act_on_user_answer(find_charging_station, num_tries, bearer_token, email, requestedModel, text_to_speech_client, speech_to_text_client)
...
def main():
    ...
    display_in_cockpit(starting_output)
